    ...
    for (i=0; i<len; i++){
        (function(script){
            ...
            var type = script.getAttribute('type') || 'text/javascript',
                ...
                matches = /^((http(s)?:)?\/\/([^\/]+)\/)embed\/([\d]+).js($|\?(.*)(#|$))/.exec(src),
                ...
                formId = matches ? parseInt(matches[5]) : 'false',
                iframeSrc = '';

            if (!target123Domain || isNaN(formId) || formId < 1 || type != 'text/javascript' || script.getAttribute('data-role') == null || script.getAttribute('data-role') != 'form') {
                return;
            }
            ...
            iframe.src = (
                iframeSrc = target123Domain
                    ...
                    + 'ref=' + encodeURIComponent(String(window.top.location.href).replace(/(<([^>]+)>)/ig, '')).replace('%26', '[%ANDCHAR%]')
                    + '&_embedType_=embed.js'
            );

            iframe.id = +new Date() + '_' + ~~(Math.random() * 100000)+ ~~(Math.random() * 100000)+ ~~(Math.random() * 100000);
            ...
            (Resizer||iFrameResize)({ heightCalculationMethod: (navigator.userAgent.indexOf("MSIE") !== -1) ? 'max' : 'documentElementOffset'}, iframe);
